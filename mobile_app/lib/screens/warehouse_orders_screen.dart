...
class _WarehouseOrdersScreenState extends State<WarehouseOrdersScreen>
  ...
  String?
  _advancingOrderId; // tracks which order is currently advancing (spinner guard)
  ...
  Future<void> _advanceNextStage(dynamic order) async {
    ...
    if (orderId == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Invalid OrderID: $rawId (type: ${rawId.runtimeType})'),
        ),
      );
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Order advanced to next stage!')),
      );
      await _loadOrders();
    } catch (e) {
      if (!mounted) return;
      ...
      children: [
        TabBar(
          ...
          tabs: [
            ...
            Tab(
              text:
                  'History (${_deliveredOrders.length + _rejectedOrders.length})',
            ),
          ],
        ),
        Expanded(
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final currentStage =
              int.tryParse(order['CurrentStage']?.toString() ?? '2') ?? 2;
          ...
          return _buildOrderCard(
            ...
            actionButton: SizedBox(
              ...
              child: ElevatedButton.icon(
                onPressed: (isDisabled || isAdvancing)
                    ? null
                    : () => _advanceNextStage(order),
                icon: isAdvancing
                    ? const SizedBox(
                        width: 18,
                        height: 18,
                ...
                style: ElevatedButton.styleFrom(
                  ...
                  disabledBackgroundColor: isAdvancing
  ...
  Widget _buildOrderCard(dynamic order, {Widget? actionButton}) {
    ...
    final deliveryDate =
        order['DeliveryDate']?.toString().split('T')[0] ?? 'N/A';
            ...
            Row(
              children: [
                Text(
                  'Status: $status',
                  style: TextStyle(
                    color: status == 'rejected' ? Colors.red : Colors.blueGrey,
                ...
                if (status != 'pending' &&
                  ...
                  Text(
                    'Stage $currentStage/7: ${_stageNames[currentStage] ?? 'Unknown'}',
                    style: const TextStyle(
...
class _HistoryTabViewState extends State<_HistoryTabView> {
  ...
  Widget _buildDeliveredList() {
    if (widget.deliveredOrders.isEmpty)
      return const Center(
        child: Text(
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final deliveryDate =
              order['DeliveryDate']?.toString().split('T')[0] ?? 'N/A';
          final orderDate = (order['CreatedAt'] ?? order['OrderDate'] ?? 'N/A')
              .toString()
              .split('T')[0];
  ...
  Widget _buildRejectedList() {
    if (widget.rejectedOrders.isEmpty)
      return const Center(
        child: Text(
          'No rejected orders',
          style: TextStyle(fontSize: 16, color: Colors.grey),
    ...
    return RefreshIndicator(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final totalPrice = order['TotalPrice']?.toString() ?? '0.00';
          final reason =
              order['RejectionReason']?.toString() ?? 'No reason provided';
