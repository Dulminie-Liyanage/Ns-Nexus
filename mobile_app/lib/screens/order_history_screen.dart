...
class _OrderHistoryScreenState extends State<OrderHistoryScreen> {
  ...
  Future<void> _showItemsDialog(dynamic orderId, String status, String reason) async {
    showDialog(
      context: context,
                  ...
                  child: items.isEmpty
                      ...
                      : ListView.builder(
                          ...
                          itemBuilder: (context, i) {
                            final item = items[i];
                            final name = item['ProductName'] ?? item['productName'] ?? 'Product';
                            final qtyRaw = item['QtyRequested'] ?? item['Quantity'] ?? 0;
                            final approvedRaw = item['QtyApproved'] ?? qtyRaw;
                            final priceRaw = item['UnitPrice'] ?? item['Price'] ?? 0.0;
                            ...
                            final isModified = qApprv < qReq && safeStatus != 'rejected';

                            return Container(
                              color: isModified ? Colors.orange.withOpacity(0.1) : null,
                              child: ListTile(
                                title: Text(name, style: TextStyle(
                                    color: isModified ? Colors.deepOrange : Colors.black87,
    ...
    try {
      ...
      if (cart.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('No items found in this order to reorder.')),
        );
        return;
      }
      ...
      Navigator.pop(context); // dismiss loading
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Failed to load order: ${e.toString().replaceFirst("Exception: ", "")}')),
      );
    }
  }
  ...
  Color _getStatusColor(String status) {
    switch (status.toLowerCase()) {
      case 'approved': case 'partially_approved': return Colors.green;
      case 'rejected': return Colors.red;
      case 'flagged_for_review': return Colors.orange;
      case 'pending': return Colors.orange;
      case 'shipped': case 'processing': return Colors.blue;
  ...
  Widget _buildProgressBar(int currentStage) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        children: List.generate(7, (index) {
          ...
          return Expanded(
            child: Container(
              ...
              height: 7,
              decoration: BoxDecoration(
                color: isActive ? const Color(0xFF0056B3) : Colors.grey.shade200,
      ...
      Text('Stage $currentStage / 7 — ${_getStageName(currentStage)}',
          style: const TextStyle(
              fontWeight: FontWeight.w600, color: Color(0xFF0056B3), fontSize: 12)),
    ]);
  }
            ...
            child: Padding(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                ...
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text('Order #$orderId',
                      style: const TextStyle(
                          fontWeight: FontWeight.w700, fontSize: 16, color: Color(0xFF1E293B))),
                  Container(
                ...
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  ...
                  Text('LKR ${order['TotalPrice']?.toString() ?? '0.00'}',
                      style: const TextStyle(
                ...
                if (order['IsUrgent'] == 1) ...[
                  ...
                  Container(
                    ...
                    child: Row(mainAxisSize: MainAxisSize.min, children: [
                      Icon(Icons.bolt_rounded, size: 12, color: Colors.red.shade700),
                      const SizedBox(width: 4),
                      Text('Urgent Order',
                          style: TextStyle(
                              fontSize: 11, color: Colors.red.shade700, fontWeight: FontWeight.w600)),
                    ]),
                  ),
                ],
                ...
                Row(children: [
                  ...
                  if (['approved', 'partially_approved', 'delivered',
                        'packing', 'in_3pl_transit', 'ready_to_ship',
                        'out_for_delivery'].contains(status.toLowerCase()))
                  Expanded(
