...
class _DeliveryScheduleScreenState extends State<DeliveryScheduleScreen> {
  ...
  // US-13: Step 2 — endDelivery() → status = "delivered"
  // US-16: Auto-sets driver status back to AVAILABLE
  Future<void> _endDelivery(DeliveryStop stop) async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('End Delivery?'),
        content: Text(
          'Confirm delivery completed for Order #${stop.orderId} — ${stop.retailerName}?',
        ),
    ...
    setState(() => _updatingId = stop.orderId);
    try {
      await _svc.markDelivered(stop.orderId);
      // US-16: Explicitly set AVAILABLE immediately — no refresh needed
      await _driverSvc.updateStatus('AVAILABLE');
      _load();
      _snack('✓ Delivery completed — status set to Available.');
    } catch (e) {
      _snack(e.toString().replaceFirst('Exception: ', ''), isError: true);
      setState(() => _updatingId = null);
    }
      ...
      builder: (_) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 14),
            _detailRow(Icons.location_on_outlined, 'Address', stop.address),
            const SizedBox(height: 14),
